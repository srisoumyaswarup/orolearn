...
function EducatorDashboard() {
  ...
  useEffect(() => {
    const pullProfileData = async () => {
      ...
      supabase.channel(`educator-notif-${user.id}`)
  ...
  return (
    <div className={`min-h-screen flex font-sans transition-colors duration-300 ${isDarkMode ? 'bg-[#0f172a] text-slate-200' : 'bg-slate-50 text-slate-800'}`}>
      ...
      <aside className={`w-20 lg:w-64 border-r flex flex-col z-20 fixed h-full transition-colors duration-300 justify-between ${isDarkMode ? 'bg-[#1e293b] border-slate-800' : 'bg-white border-slate-200'}`}>
        <div>
          {/* CORRECTED ONCLICK ROUTE LOGIC FOR THE MAIN WORKSPACE LOGO ELEMENT */}
          ...
          <nav className="px-3 space-y-1">
            ...
            <NavItem icon={Icons.schedule} label="Schedule" active={activeTab === 'schedule'} onClick={() => setActiveTab('schedule')} isDark={isDarkMode} />
            <NavItem icon={Icons.video} label="Live Studio" active={activeTab === 'online_class'} onClick={() => setActiveTab('online_class')} isDark={isDarkMode} />
            ...
            <NavItem icon={Icons.wallet} label="Earnings" active={activeTab === 'earnings'} onClick={() => setActiveTab('earnings')} isDark={isDarkMode} />
      ...
      <main className="flex-1 ml-20 lg:ml-64 overflow-y-auto min-h-screen">
        <header className={`sticky top-0 z-30 border-b px-8 py-4 flex justify-between items-center ${isDarkMode ? 'bg-[#0f172a]/90 border-slate-800' : 'bg-white/90 border-slate-200'} backdrop-blur-md`}>
          <h2 className="text-sm font-semibold">Good Day, {profile.full_name.split(' ')[0]}</h2>
               ...
               {showNotifDropdown && (
                 <div className={`absolute top-10 right-0 w-80 rounded-xl shadow-2xl border py-3 z-50 max-h-[300px] overflow-y-auto text-left ${isDarkMode ? 'bg-[#1e293b] border-slate-700' : 'bg-white border-slate-200'}`}>
                   <div className="px-4 pb-2 border-b font-bold text-xs flex justify-between items-center">
                     <span>Student Booking Alerts</span>
                   ...
                   {notifications.length === 0 ? (
                     <div className="p-4 text-center text-xs text-slate-400">No active student alerts reported.</div>
                   ) : (
                     notifications.map(n => (
        ...
        <div className="p-8 max-w-6xl mx-auto space-y-6 relative">
          {activeTab === 'overview' && (
            <div className={`grid grid-cols-1 lg:grid-cols-3 gap-6 ${!isVerified ? 'z-10 relative' : ''}`}>
              <div className={`lg:col-span-2 rounded-2xl border p-6 shadow-sm flex flex-col justify-between min-h-[220px] ${isDarkMode ? 'bg-[#1e293b] border-slate-700' : 'bg-white border-slate-200'}`}>
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">Educator Badge</h3>
                  <span className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 px-3 py-1 rounded-full text-xs font-semibold">Verified</span>
                </div>
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                  <div className="w-24 h-24 rounded-full border-4 overflow-hidden shrink-0 shadow-md">
                  ...
                  <div className="text-center sm:text-left flex-1">
                    <h2 className="text-2xl font-bold tracking-tight mb-1">{profile.full_name}</h2>
                    <p className="text-[#C9A85E] font-medium text-sm mb-4">{profile.academic?.subjects?.[0] || 'Expert Mentor'}</p>
                    <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                      <span className="px-3 py-1 rounded-lg text-xs font-medium border bg-slate-50 dark:bg-slate-800">{profile.oro_badge_id}</span>
                      <span className="px-3 py-1 rounded-lg text-xs font-medium border bg-slate-50 dark:bg-slate-800">{calculateAge(profile.dob)} Yrs • {profile.gender}</span>
                    </div>
                  </div>
                  <div onClick={() => setIsQrModalOpen(true)} className="p-2 rounded-xl border bg-slate-50 dark:bg-slate-800 cursor-pointer">
                    <img src={`https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=${profile.oro_badge_id}&color=C9A85E&bgcolor=${isDarkMode ? '1e293b' : 'ffffff'}`} className="w-16 h-16 rounded-lg opacity-80" alt="QR" />
                  </div>
                </div>
              </div>
              <div className="flex flex-col gap-6">
                <StatCard label="Subject Rank" value="#12" isDark={isDarkMode} />
                <StatCard label="ORO Points" value="8,450" color="text-[#C9A85E]" isDark={isDarkMode} />
              </div>
            </div>
          )}
          ...
          <div className="relative">
            ...
            {activeTab === 'overview' && (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <StatCard label="Total Earnings" value="₹12,450" isDark={isDarkMode} />
                <StatCard label="Active Students" value="142" isDark={isDarkMode} />
                <StatCard label="Avg. Rating" value="⭐ 4.9" isDark={isDarkMode} />
                <StatCard label="Performance Score" value="94%" isDark={isDarkMode} />
              </div>
            )}
            ...
            {activeTab === 'chat' && (
              <div className={`rounded-2xl border p-8 text-center ${isDarkMode ? 'bg-[#1e293b] border-slate-800' : 'bg-white border-slate-200'}`}>
                ...
                <p className="text-slate-500 text-sm max-w-md mx-auto">When a student subscribes to your teaching node, a real-time messaging pipeline mounts here instantly for coordinate management without leaking personal phone lines.</p>
              </div>
            )}
      ...
      {isQrModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" onClick={() => setIsQrModalOpen(false)}>
          <div className={`rounded-3xl p-8 shadow-2xl flex flex-col items-center max-w-sm w-full ${isDarkMode ? 'bg-[#1e293b]' : 'bg-white'}`} onClick={e => e.stopPropagation()}>
            <img src={`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${profile.oro_badge_id}&color=C9A85E&bgcolor=${isDarkMode ? '1e293b' : 'ffffff'}`} alt="Educator QR Code" className="w-48 h-48 rounded-xl border p-2" />
